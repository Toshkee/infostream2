...
/* ─── Clients — a proof-strip of real institutional marks ───────────────
   The grid intentionally uses actual client marks rather than illustrative
   glyphs. It is the quick trust signal; the domain pages remain the place for
   the detailed organisation — system register. */

type ClientLogo = { src: string; label: string; className?: string };
...
function featuredLogos(lang: Locale): ClientLogo[] {
  ...
  return [
    ...
    { src: "/clients/tax-administration.transparent.png", label: mne ? "Poreska uprava" : "Tax Administration" },
    ...
    { src: "/clients/official-gazette.transparent.png", label: mne ? "Službeni list Crne Gore" : "Official Gazette of Montenegro" },
    ...
    { src: "/clients/eu.transparent.png", label: "European Union / CFCU", className: "max-h-12 max-w-[8.5rem]" },
    { src: "/clients/cultural-heritage.transparent.png", label: mne ? "Uprava za zaštitu kulturnih dobara" : "Cultural Heritage Administration" },
    { src: "/clients/ministry-regional-development.transparent.png", label: mne ? "Ministarstvo regionalno-investicionog razvoja" : "Ministry of Regional and Investment Development" },
    { src: "/clients/montenegro-coa.transparent.png", label: mne ? "Vlada Crne Gore" : "Government of Montenegro" },
  ];
}
      ...
      <div className={`proof-strip-track${reverse ? " proof-strip-track-reverse" : ""}`}>
        {[false, true].map((copy) => (
          <div key={String(copy)} className="proof-strip-group" aria-hidden={copy || undefined}>
            {logos.map((logo) => (
              <div key={`${copy}-${logo.src}`} className="proof-strip-logo">
                <Image
                  src={logo.src}
                  alt={copy ? "" : logo.label}
                  width={180}
...
const LOGO_RULES: [RegExp, { src: string; cover?: boolean }][] = [
  ...
  [/tax administration|poreska uprava/, { src: "/clients/tax-administration.transparent.png" }],
  ...
  [/gazette|službeni list/, { src: "/clients/official-gazette.transparent.png" }],
