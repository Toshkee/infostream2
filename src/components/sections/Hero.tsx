  ...
  return (
    <section className="relative min-h-[100svh] overflow-hidden bg-[var(--bg-inset)] text-white">
      {/* Quiet dark backdrop — deliberately free of the planets/orbits, which
         are the process section's visual identity further down the page. The
         lamp above the headline is the one light source. */}
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-[1280px] flex-col items-center justify-center px-6 pt-20 pb-8 text-center md:pt-24 md:pb-12 lg:px-10">
        {/* Lamp (after Aceternity UI's "Lamp"): a teal bar casting a soft
           cone down over the headline. Pure CSS (`.hero-lamp` in
           globals.css) so it opens on the first painted frame, like the rise. */}
        <div aria-hidden className="hero-lamp">
          <div className="hero-lamp-cone" />
          <div className="hero-lamp-glow" />
          <div className="hero-lamp-bar" />
        </div>

        <div className="hero-rise text-[10px] font-medium tracking-[0.2em] uppercase text-[var(--brand-teal-bright)] flex items-center justify-center gap-3 sm:text-[11px] sm:tracking-[0.25em]">
          <EyebrowBars />
        ...
        <h1 className="font-display mt-6 text-[clamp(2.2rem,5.2vw,4.8rem)] leading-[1.02] tracking-[-0.025em] font-medium text-white max-w-4xl text-balance">
          {titleWords.map((w, i) => (
            <span
              key={i}
              className="hero-rise hero-lit inline-block mx-[0.11em]"
        ...
        <p
          className="hero-rise mt-6 max-w-xl text-white/70 text-pretty text-[16.5px] leading-relaxed"
        ...
        <div className="mt-12 grid w-full max-w-3xl grid-cols-2 gap-x-8 gap-y-6 sm:mt-14 sm:grid-cols-4 sm:gap-x-10 sm:gap-y-7">
          {hero.meta.map((m, i) => (
            <div
              key={i}
